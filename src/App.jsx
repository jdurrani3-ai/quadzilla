...
function extractYouTubeId(raw) {
  ...
  const pp = [/[?&]v=([a-zA-Z0-9_-]{11})/,/youtu\.be\/([a-zA-Z0-9_-]{11})/,/\/embed\/([a-zA-Z0-9_-]{11})/,/\/live\/([a-zA-Z0-9_-]{11})/,/\/shorts\/([a-zA-Z0-9_-]{11})/];
...
const ytEmbed   = (id, muted) => `https://www.youtube-nocookie.com/embed/${id}?autoplay=1&mute=${muted?1:0}&controls=1&rel=0&modestbranding=1`;
const kickEmbed = (ch, muted) => `https://player.kick.com/${ch}?autoplay=true&muted=${muted}`;
...
/* ── Layouts  ─  ALWAYS use minmax(0,1fr) so rows can shrink ── */
const LAYOUTS = {
...
/* ════════════════════════════════════════════════
   DROPDOWN  — rendered as a portal on <body>
   so panel overflow:hidden cannot clip it.
════════════════════════════════════════════════ */
function Dropdown({ open, pos, color, presets, panelId, onPick, onClose }) {
  if (!open || !pos) return null;
  return createPortal(
    <>
      {/* invisible overlay catches outside taps */}
      <div onClick={onClose} onTouchStart={onClose}
        style={{ position:"fixed", inset:0, zIndex:8888, background:"transparent" }}/>
      <div style={{ position:"fixed", top:pos.bottom+4, left:pos.left,
                    zIndex:9999, background:"#0C0C12",
                    border:`1px solid ${color}55`, borderRadius:"4px",
                    minWidth:"170px", maxHeight:"240px", overflowY:"auto",
          ...
          {APPS.map(app=>(
            <div key={app.name} onClick={()=>{ window.open(app.url,"_blank"); onClose(); }}
              ...
              <img src={app.icon} alt={app.name}
                style={{ width:"30px", height:"30px", borderRadius:"6px", objectFit:"contain", flexShrink:0 }}/>
              {app.name}
            </div>
          ))}
        </>)}
...
function Panel({ panelId, data, onUpdate, extraStyle, isMobile }) {
  ...
  const numSz = isMobile ? "15px"  : "13px";
  ...
  const IBtn = ({onClick, children, style={}}) => (
    <div onClick={onClick}
      style={{ width:btnSz, height:btnSz, display:"flex", alignItems:"center",
               ...
               userSelect:"none", transition:"all 0.12s", ...style }}>
      {children}
    </div>
  );
  ...
  return (
    /*
      CRITICAL:  overflow:"hidden" stays on the panel so grid rows
      are properly constrained.  The dropdown escapes via portal.
    */
    <div style={{ display:"flex", flexDirection:"column", minHeight:0,
      ...
      <div style={{ display:"flex", alignItems:"center", gap:isMobile?"7px":"5px",
        ...
        {/* Panel number */}
        <span style={{ fontFamily:"'Bebas Neue',cursive", fontSize:numSz,
                       color:hasStream?cfg.color:"#1A1A22",
                       letterSpacing:"1px", minWidth:"20px", lineHeight:1,
                       transition:"color 0.3s", flexShrink:0 }}>
          {String(panelId).padStart(2,"0")}
        </span>

        {/* Dropdown button ─ getBoundingClientRect gives portal its position */}
        <div ref={btnRef} style={{ flexShrink:0 }}>
          <IBtn onClick={openDropdown}
            style={{ border:`1px solid ${cfg.color}55`,
                     background:dropdown?cfg.color+"18":"transparent" }}>
        ...
        {hasStream && (<>
          ...
          <IBtn onClick={clear}
            style={{ border:"1px solid #1C1C28", color:"#2E2E3E" }}>
            <CloseIcon/>
          </IBtn>
        </>)}
      </div>

      {/* Content — overflow:hidden here contains iframes */}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    /* Global CSS
       100dvh = iOS Safari dynamic viewport height (excludes address bar)
       overflow hidden on html/body prevents bounce scroll */
    const style=document.createElement("style");
    style.id="qz-g";
    style.textContent=`
      *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
      html{height:100dvh;overflow:hidden;}
      body{height:100dvh;overflow:hidden;background:#06060A;}
    ...
    const check=()=>{
      ...
      const mob=w<768;
      setIsMobile(mob);
      setIsPortrait(mob&&w<h);
      setIsLandscape(mob&&w>h);
    };
    check();
  ...
  /* Portrait: single column, 4 rows
     Landscape: 4 columns, 1 row
     — always use minmax(0,1fr) so rows shrink to fit ── */
  const gridStyle=(()=>{
    if(isPortrait&&layout==="2x2")
      return{gridTemplateColumns:"1fr",gridTemplateRows:"minmax(0,1fr) minmax(0,1fr) minmax(0,1fr) minmax(0,1fr)"};
    if(isLandscape&&layout==="2x2")
      return{gridTemplateColumns:"1fr 1fr 1fr 1fr",gridTemplateRows:"minmax(0,1fr)"};
    return cfg.grid;
  })();
    ...
    <div style={{ width:"100vw",height:"100dvh",background:"#06060A",
      ...
      <div style={{ height:headerH,flexShrink:0,display:"flex",alignItems:"center",
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"7px" }}>
          <div style={{ width:"6px",height:"6px",borderRadius:"50%",background:"#E8FF00",
                        boxShadow:"0 0 8px #E8FF00",flexShrink:0 }}/>
          <span style={{ fontFamily:"'Bebas Neue',cursive",fontSize:logoFs,
                         color:"#E8FF00",letterSpacing:"7px",lineHeight:1 }}>QUADZILLA</span>
          {!isMobile&&<span style={{ fontFamily:"'IBM Plex Mono',monospace",fontSize:"7px",
                                     color:"#131320",letterSpacing:"3px" }}>MULTI·STREAM</span>}
          {liveCount>0&&<div style={{ fontFamily:"'IBM Plex Mono',monospace",fontSize:"7px",
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"3px" }}>
          {!isMobile&&<span style={{ fontFamily:"'IBM Plex Mono',monospace",fontSize:"7px",
                                     color:"#111120",letterSpacing:"2px",marginRight:"4px" }}>LAYOUT</span>}
      ...
      {/* Grid
          flex:"1 1 0" + minHeight:0 forces the grid to never exceed
          remaining viewport height regardless of panel content */}
      <div style={{ flex:"1 1 0",minHeight:0,display:"grid",
                    gap:"2px",padding:"2px",overflow:"hidden",...gridStyle }}>
        {cfg.visible.map(id=>(
